...
export class DSLRenderer {
    ...
    syncMaterials(materials) {
        const currentIds = new Set(materials.map((mat) => mat.id).filter(Boolean));
    ...
    // 创建ThreeJS材质
    createThreeMaterial(dslMaterial) {
        let material;
    ...
    // 更新ThreeJS材质
    updateThreeMaterial(threeMaterial, dslMaterial) {
    ...
    // 更新ThreeJS对象
    updateThreeObject(threeObject, dslObject) {
        // 更新变换
    ...
    createGeometry(geomDef) {
        ...
        switch (geom.type) {
            case 'box':
                return new THREE.BoxGeometry(geom.size?.[0] || 1, geom.size?.[1] || 1, geom.size?.[2] || 1, geom.segments?.[0] || 1, geom.segments?.[1] || 1, 1);
            case 'sphere':
                return new THREE.SphereGeometry(geom.radius || 1, geom.radialSegments || 16, geom.heightSegments || 12, 0, Math.PI * 2, 0, Math.PI);
            case 'plane':
                return new THREE.PlaneGeometry(geom.size?.[0] || 1, geom.size?.[1] || 1, geom.segments?.[0] || 1, geom.segments?.[1] || 1);
            case 'cylinder':
                return new THREE.CylinderGeometry(geom.radius || 1, geom.radius || 1, geom.height || 1, geom.radialSegments || 8, geom.heightSegments || 1);
            default:
                return new THREE.BoxGeometry(1, 1, 1);
        }
    }
    // 创建光源
    createThreeLight(dslLight) {
        let light;
        switch (dslLight.type) {
            ...
            case 'directional': {
                ...
                if (dslLight.position) {
                    if (Array.isArray(dslLight.position)) {
                        light.position.set(dslLight.position[0] || 0, dslLight.position[1] || 0, dslLight.position[2] || 0);
                    }
                    else {
            ...
            case 'point': {
                const pointLight = new THREE.PointLight(dslLight.color || '#ffffff', dslLight.intensity || 1);
                if (dslLight.position) {
                    if (Array.isArray(dslLight.position)) {
                        pointLight.position.set(dslLight.position[0] || 0, dslLight.position[1] || 0, dslLight.position[2] || 0);
                    }
                    else {
            ...
            case 'spot': {
                ...
                if (dslLight.position) {
                    if (Array.isArray(dslLight.position)) {
                        spotLight.position.set(dslLight.position[0] || 0, dslLight.position[1] || 0, dslLight.position[2] || 0);
                    }
                    else {
                ...
                if (dslLight.target) {
                    if (Array.isArray(dslLight.target)) {
                        spotLight.lookAt(dslLight.target[0] || 0, dslLight.target[1] || 0, dslLight.target[2] || 0);
                    }
                    else {
    ...
    // 更新光源
    updateThreeLight(threeLight, dslLight) {
        ...
        if (dslLight.position && 'position' in threeLight) {
            if (Array.isArray(dslLight.position)) {
                threeLight.position.set(dslLight.position[0] || 0, dslLight.position[1] || 0, dslLight.position[2] || 0);
            }
            else {
        ...
        if (dslLight.target && 'lookAt' in threeLight) {
            if (Array.isArray(dslLight.target)) {
                threeLight.lookAt(dslLight.target[0] || 0, dslLight.target[1] || 0, dslLight.target[2] || 0);
            }
            else {
                threeLight.lookAt(dslLight.target);
            }
        }
        // 更新特定光源属性
    ...
    syncCamera(dslCamera) {
        ...
        else if (dslCamera.type === 'orthographic' &&
        ...
        if (Array.isArray(dslCamera.position)) {
            this.camera.position.set(dslCamera.position[0] || 0, dslCamera.position[1] || 0, dslCamera.position[2] || 0);
        }
        else {
            this.camera.position.copy(dslCamera.position);
        }
        if (Array.isArray(dslCamera.target)) {
            this.camera.lookAt(dslCamera.target[0] || 0, dslCamera.target[1] || 0, dslCamera.target[2] || 0);
        }
        else {
    ...
    getMaterial(material) {
        ...
        // 为内联材质创建临时材质
        return this.createThreeMaterial(material);
    }
    // 开始渲染循环
    startRenderLoop() {
        const animate = () => {
